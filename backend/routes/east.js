...
router.get('/notes/:number(\\d+)/', async (req, res) => {
   try {
      ...
      const numberNote = data[2]['notes'][int_number];
      if (numberNote == null) {
...
router.post('/notes', async (req, res) => {
   const postNumber = data[2]['notes'].length
   const note = new Note({
...
router.delete('/notes/:postNumber(\\d+)/', async(req, res) => {
   try {
      ...
      if (postNumber != 0) {
         const toDelete = data[2]['notes'][postNumber];
         data[2]['notes'].splice(postNumber, 1);
         res.json(toDelete);
      }
   } catch (e) {
      console.log(e);
...
router.patch('/notes/:postNumber(\\d+)/', async (req, res) => {
   try {
      const postNumber = parseInt(req.params.postNumber);
      data[2]['notes'][postNumber]['text'] = req.body.text;
      const updatedPost = data[2]['notes'][postNumber];
