...
router.get('/notes', async (req, res) => {
   try {
      const allNotes = data['sensors'][2]['notes'];
...
router.get('/notes/:number(\\d+)/', async (req, res) => {
   try {
      ...
      const numberNote = data['sensors'][2]['notes'][int_number - 1];
      if (numberNote == null) {
...
router.post('/notes', async (req, res) => {
   const postNumber = data['sensors'][2]['notes'].length + 1
   const note = new Note({
      number: postNumber.toString(),
      text: req.body.text
   });
   try {
      data['sensors'][2]['notes'].push(note);
      res.json(note);
...
router.delete('/notes/:postNumber(\\d+)/', async(req, res) => {
   try {
      ...
      const toDelete = data['sensors'][2]['notes'][postNumber - 1];
      data['sensors'][2]['notes'].splice(postNumber - 1, 1);
      res.json(toDelete);
   } catch (e) {
      console.log(e);
      console.log(res.status);
...
router.patch('/notes/:postNumber(\\d+)/', async (req, res) => {
   try {
      const postNumber = parseInt(req.params.postNumber);
      data['sensors'][2]['notes'][postNumber - 1]['text'] = req.body.text;
      const updatedPost = data['sensors'][2]['notes'][postNumber - 1];
